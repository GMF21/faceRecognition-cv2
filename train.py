...
# ----------------------------
# Função universal para detetar webcam
# ----------------------------
...
def show_welcome_screen(screen, font, message, duration=2):
    """Mostra uma tela de boas-vindas limpa durante `duration` segundos."""
    ...
    while time.time() - start_time < duration:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                sys.exit()
        screen.fill(COLORS['bg'])
        text_surface = font.render(message, True, COLORS['text'])
...
def main():
    ...
    while running:
        ...
        # RECONHECIMENTO DURANTE 10 SEGUNDOS
        if timer_start and (time.time() - timer_start < 10) and face_recognizer:
            ...
            for (x, y, w, h) in faces:
                ...
                name_display = recognized_name if recognized_name else "Desconhecido"
                cv2.putText(frame, name_display, (x, y-10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0,255,0), 2)

            # Se reconheceu, mostrar boas-vindas e tocar som
            if recognized_name:
                if unlock_sound:
                    unlock_sound.play()
                show_welcome_screen(screen, font, f"Bem-vindo {recognized_name}", duration=2)
                recognized_name = None
                timer_start = None
        ...
        # Mensagem de erro PIN
        if pin_error:
            screen.blit(font.render("Código incorreto!", True, (255,0,0)), (200, 400))
